...
export default function ExamGradingDetail() {
  ...
  // 1. 获取考试提交详情
  const { data, isLoading } = trpc.exams.getSubmissionDetail.useQuery({
    submissionId: sId,
  });
  ...
  if (!data)
    return <div className="p-12 text-center font-bold">未找到相关考试记录</div>;
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-black uppercase">
                考试批阅：{data.submission.studentName}
              </h1>
              <p className="text-xs text-zinc-400 font-bold">
                {data.submission.examTitle}
              </p>
            </div>
          </div>
          <Button
            ...
            className="bg-zinc-900 hover:bg-black text-white gap-2 rounded-xl px-6 h-11 font-bold"
          >
              ...
              >
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-3">
                    <Badge className="bg-zinc-100 text-zinc-500 border-none uppercase text-[9px] font-black">
                      {q.type}
                    </Badge>
                    {isObjective && (
                      <div
                        className={`flex items-center gap-1 text-[10px] font-bold ${q.isCorrect ? "text-emerald-500" : "text-rose-500"}`}
                      >
                        {q.isCorrect ? (
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      onChange={e =>
                        setScores({
                          ...scores,
                          [q.detailId]: parseFloat(e.target.value) || 0,
                        })
                      }
                      className={`w-20 rounded-lg px-2 py-1 text-center font-black text-sm outline-none border-2 transition-all ${
                        isObjective && q.isCorrect
                          ? "bg-emerald-50 border-emerald-100"
                          : "bg-zinc-50 border-transparent focus:border-zinc-900"
                      }`}
                    />
                ...
                <div className="grid md:grid-cols-2 gap-6 pt-2">
                  ...
                  <div className="space-y-2">
                    <p className="text-[10px] font-black uppercase text-emerald-500/50 tracking-widest">
                      标准答案及解析
                    </p>
                    <div className="p-6 bg-emerald-50/30 rounded-[2rem] border border-emerald-100/50">
                      <div className="mb-4">
                        <div className="text-[10px] font-black text-emerald-600/50 uppercase mb-2 tracking-tighter">
                          Correct Answer
                        </div>

                        {/* ⚡️ 判断是否为编程题或包含换行符 */}
                        {q.type === "programming" ||
                        q.standardAnswer?.includes("\n") ? (
                          <pre className="font-mono text-xs leading-relaxed text-emerald-800 bg-white/50 p-4 rounded-xl border border-emerald-100/30 overflow-x-auto whitespace-pre-wrap">
                            {q.standardAnswer}
                          </pre>
                        ) : (
                      ...
                      {q.analysis && (
                        <div className="pt-4 border-t border-emerald-100/30">
                          <div className="text-[10px] font-black text-emerald-600/40 uppercase mb-1 tracking-tighter">
                            Analysis
                          </div>
